...
public static class TypeConverterExtension
{
    ...
    // FolkerKinzel.CsvTools.Mappings.TypeConverters namespace cannot always been
    // published because some type names conflict with System.Component.Model
    ...
    /// <see cref="TypeConverter{T}.DefaultValue"/> of this <see cref="TypeConverter{T}"/> 
    /// instance.
    /// </summary>
    ...
    /// <see cref="DBNull.Value"/> is the <see cref="TypeConverter{T}.DefaultValue"/> of this 
    /// new <see cref="TypeConverter{T}"/> instance.
    /// </returns>
    /// 
    /// <remarks>
    /// <para>
    ...
    /// <see cref="ToNullableConverter{T}(TypeConverter{T})"/> extension method for value type 
    /// converters to create a <see cref="TypeConverter{T}"/> instance whose 
    /// <see cref="ITypeConverter{T}.DefaultValue"/> is <c>null</c>.
    /// </para>
    /// </remarks>
    /// 
    /// <example>
    ...
    /// </example>
    /// 
    /// <exception cref="ArgumentNullException"><paramref name="converter"/> is <c>null</c>.
    /// </exception>
    /// <exception cref="ArgumentException">
    /// The <see cref="ITypeConverter{T}.DefaultValue"/> of <paramref name="converter"/> is
    /// not <c>null</c>.
    /// </exception>
    ...
    /// <param name="converter">The <see cref="TypeConverter{T}"/> instance that is used as 
    /// template.</param>
    /// <returns>The newly created <see cref="TypeConverter{T}"/> instance that converts 
    /// <see cref="Nullable{T}"/> instead of <typeparamref name="T"/>.</returns>
    /// 
    /// <example>
    /// <note type="note">In the following code examples - for easier readability - exception 
    /// handling has been omitted.</note>
    ...
    /// </summary>
    /// <typeparam name="TItem">The <see cref="Type"/> of the items of the 
    /// <see cref="IEnumerable{T}"/> objects that the newly created converter can convert.
    /// </typeparam>
    /// <param name="itemsConverter">A <see cref="TypeConverter{T}"/> instance that converts
    /// the items.</param>
    /// <param name="separator">A <see cref="string"/> that separates the items in a field 
    /// of the CSV file. When parsing the CSV, <paramref name="separator"/> will not be part 
    /// of the results.</param>    
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="TypeConverter{T}"/> instance that converts <see cref="IEnumerable{T}"/>.
    /// The value of its <see cref="TypeConverter{T}.Throwing"/> property is derived from 
    /// <paramref name="itemsConverter"/>.
    /// </returns>
    /// 
    /// <remarks>
    /// The converter the method creates uses a simple string split and join operation. For 
    /// nested CSV better use a simple <see cref="StringConverter"/> and initialize a separate 
    /// <see cref="CsvReader"/>, <see cref="CsvWriter"/>, and <see cref="CsvMapping"/> to 
    /// handle the nested items.
    /// </remarks>
    /// 
    /// <exception cref="ArgumentNullException"><paramref name="itemsConverter"/> or 
    /// <paramref name="separator"/> is <c>null</c>.</exception>
    /// <exception cref="ArgumentException"><paramref name="separator"/> is an 
    /// <see cref="string.Empty"/>.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<IEnumerable<TItem?>?> 
        ToIEnumerableConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                                      string separator,
                                      bool nullable = true)
    ...
    /// </summary>
    /// <typeparam name="TItem">The <see cref="Type"/> of the items of the 
    /// <see cref="IList{T}"/> objects that the newly created converter can convert.
    /// </typeparam>
    /// <param name="itemsConverter">A <see cref="TypeConverter{T}"/> instance that 
    /// converts the items.</param>
    /// <param name="separator">A <see cref="string"/> that separates the items in a 
    /// field of the CSV file. When parsing the CSV, <paramref name="separator"/> will
    /// not be part of the results.</param>    
    /// <param name="nullable"><c>true</c> to set <see cref="TypeConverter{T}.DefaultValue"/>
    /// to <c>null</c>; <c>false</c> to have an empty <see cref="IList{T}"/> as 
    /// <see cref="TypeConverter{T}.DefaultValue"/>.
    /// </param>
    /// <returns>
    /// A new <see cref="TypeConverter{T}"/> instance that converts <see cref="IList{T}"/>.
    /// The value of its <see cref="TypeConverter{T}.Throwing"/> property is derived from 
    /// <paramref name="itemsConverter"/>.
    /// </returns>
    /// 
    ...
    /// The converter the method creates uses a simple string split and join operation. For 
    /// nested CSV better use a simple <see cref="StringConverter"/> and initialize a separate
    /// <see cref="CsvReader"/>, <see cref="CsvWriter"/>, and <see cref="CsvMapping"/> to handle
    /// the nested items.
    /// </remarks>
    /// 
    /// <exception cref="ArgumentNullException"><paramref name="itemsConverter"/> or 
    /// <paramref name="separator"/> is <c>null</c>.</exception>
    /// <exception cref="ArgumentException"><paramref name="separator"/> is an <see cref="string.Empty"/>.
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<IList<TItem?>?> 
        ToIListConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                                string separator,
                                bool nullable = true)
    ...
    /// When parsing the CSV, <paramref name="separator"/> will not be part of the results.</param>    
    /// <param name="nullable"><c>true</c> to set <see cref="TypeConverter{T}.DefaultValue"/>
    /// to <c>null</c>; <c>false</c> to have an empty <see cref="ICollection{T}"/> as 
    /// <see cref="TypeConverter{T}.DefaultValue"/>.
    /// </param>
    /// <returns>
    /// A new <see cref="TypeConverter{T}"/> instance that converts <see cref="ICollection{T}"/>. The value
    /// of its <see cref="TypeConverter{T}.Throwing"/> property is derived from <paramref name="itemsConverter"/>.
    /// </returns>
    /// 
    /// <remarks>
    /// The converter the method creates uses a simple string split and join operation. For nested CSV better 
    /// use a simple <see cref="StringConverter"/> and initialize a separate <see cref="CsvReader"/>, 
    /// <see cref="CsvWriter"/>, and <see cref="CsvMapping"/> to handle the nested items.
    /// </remarks>
    /// 
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<ICollection<TItem?>?> 
        ToICollectionConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                                      string separator,
                                      bool nullable = true)
    ...
    /// objects that the newly created converter can convert.</typeparam>
    /// <param name="itemsConverter">A <see cref="TypeConverter{T}"/> instance that converts the items.</param>
    /// <param name="separator">A <see cref="string"/> that separates the items in a field of the CSV file. When
    /// parsing the CSV, <paramref name="separator"/> will not be part of the results.</param>    
    /// <param name="nullable"><c>true</c> to set <see cref="TypeConverter{T}.DefaultValue"/> to <c>null</c>; 
    /// <c>false</c> to have an empty <see cref="IReadOnlyList{T}"/> as <see cref="TypeConverter{T}.DefaultValue"/>.
    /// </param>
    /// <returns>
    ...
    /// <exception cref="ArgumentException"><paramref name="separator"/> is an <see cref="string.Empty"/>.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<IReadOnlyList<TItem?>?> 
        ToIReadOnlyListConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                                        string separator,
                                        bool nullable = true)
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="TypeConverter{T}"/> instance that converts <see cref="IReadOnlyCollection{T}"/>. The 
    /// value of its <see cref="TypeConverter{T}.Throwing"/> property is derived from <paramref name="itemsConverter"/>.
    /// </returns>
    /// 
    /// <remarks>
    /// The converter the method creates uses a simple string split and join operation. For nested CSV better use a
    ...
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<List<TItem?>?> 
        ToListConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                               string separator,
                               bool nullable = true)
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="TypeConverter{T}"/> instance that converts <see cref="ReadOnlyCollection{T}"/>. The value
    /// of its <see cref="TypeConverter{T}.Throwing"/> property is derived from <paramref name="itemsConverter"/>.
    /// </returns>
    /// 
    /// <remarks>
    ...
    /// <exception cref="ArgumentException"><paramref name="separator"/> is an <see cref="string.Empty"/>.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TypeConverter<ReadOnlyCollection<TItem?>?>
        ToReadOnlyCollectionConverter<TItem>(this TypeConverter<TItem?> itemsConverter,
                                             string separator,
                                             bool nullable = true)
