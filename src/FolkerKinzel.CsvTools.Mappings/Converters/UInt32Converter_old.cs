...
[CLSCompliant(false)]
public sealed class UInt32Converter(IFormatProvider? formatProvider = null, bool throwing = true)
    : TypeConverter<uint>(default, throwing), IHexConverter<uint>
{
    ...
    /// <inheritdoc/>
    public object Clone() => new UInt32Converter(_formatProvider, Throwing);
    ...
    { 
        ...
        result = default;
        return !value.IsWhiteSpace() &&  uint.TryParse(value.ToString(), _styles, _formatProvider, out result);
#else
        return uint.TryParse(value, _styles, _formatProvider, out result);
#endif
    }
}
